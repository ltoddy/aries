...
use crate::mcp::config::{HttpConfig, SseConfig, StdioConfig};
use crate::mcp::{
    McpConfig, McpConnectError, McpConnectResult, McpServerConfig, NamespacedMcpTool,
};
...
impl McpManager {
    pub async fn connect(config: McpConfig) -> (Self, Vec<Box<dyn ToolDyn>>) {
        ...
        for (server_name, server_config) in config.mcp_servers {
            match Self::connect_one(&server_name, server_config).await {
