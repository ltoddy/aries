通过当前平台的 shell 执行单条命令，并返回 stdout、stderr、exit_code。在 Windows 上使用 PowerShell（`pwsh -NoProfile -NoLogo -Command` 或 `powershell.exe`，可通过 `ARIES_SHELL` 环境变量覆盖），在 Unix 上使用 `$SHELL -c`。

工作目录在命令间持久保留，但 shell 状态（变量、别名、函数）不持久。Shell 环境不加载用户 profile，以避免干扰。

可选参数 `timeout`（毫秒）用于设置命令超时，最长 600000ms（10 分钟），默认 120000ms（2 分钟）超时。超时命令会被终止并返回超时错误。stdout/stderr 超过 30000 字符时会被截断，并在末尾提示被截断的行数。可选参数 `description` 用于简短描述命令用途（5-10 个词）。不要使用需要交互式输入的命令。

重要：避免使用本工具执行 `Get-ChildItem`、`Select-String`、`Get-Content`、`Set-Content`、等命令，除非用户明确要求，或你已确认专用工具无法完成任务。请优先使用对应的专用工具，这会带来更好的体验：

 - 文件搜索：使用 Glob（不是 Get-ChildItem 或 dir）
 - 内容搜索：使用 Grep（不是 Select-String 或 findstr）
 - 读取文件：使用 Read（不是 Get-Content / type）
 - 编辑文件：使用 Edit（不是 Set-Content / (Get-Content ...) -replace ... | Set-Content）
 - 创建文件：使用 Write（不是 Set-Content / Out-File / "..." > file）
 - 输出内容：直接输出文本（不是 Write-Output / echo）

虽然 Bash 工具也能做类似的事，但优先使用内置专用工具会带来更好的体验，也更便于审查工具调用与授予权限。

# 使用说明

 - 如果命令会创建新目录或文件，先用本工具运行 `Test-Path` 或 `Get-ChildItem` 确认父目录存在且位置正确
 - 命令中包含空格的文件路径务必用双引号包裹（例如 `cd "path with spaces\file.txt"`），并 prefer 使用反斜杠 `\` 作为 Windows 路径分隔符；PowerShell 也接受正斜杠 `/`
 - 尽量在整个会话中使用绝对路径、避免使用 `cd` / `Set-Location`，以保持当前工作目录不变。仅当用户明确要求时才使用 `cd`
 - 可指定可选的 `timeout`（毫秒，最长 600000ms / 10 分钟）。默认命令在 120000ms（2 分钟）后超时
 - 执行多条命令时：
   - 如果命令相互独立、可并行，就在同一条消息里发起多个 Bash 工具调用。例如需要运行 "git status" 和 "git diff" 时，在一条消息里并行发起两个 Bash 调用
   - 如果命令相互依赖、必须顺序执行，用单个 Bash 调用并以 `;` 连接（PowerShell 7+ 也支持 `&&`/`||`）
...
注意事项：
 ...
 - 为确保格式正确，务必用 PowerShell here-string 传递 commit message，示例如下：
