...
// --- cross-platform: BashArgs::title() is shell-agnostic ---

#[test]
fn test_bash_args_title() {
...
// --- Unix / bash-specific execution & rewrite tests ---
...
mod unix {
    ...
    async fn test_bash_output_truncation() {
        ...
        let result =
            tool.call(&mut context, bash_args("printf 'a%.0s' $(seq 1 40000)")).await.unwrap();
...
//
// These exercise the BashTool end-to-end on Windows using the default shell
// (PowerShell when available, otherwise cmd.exe). They guard the cross-platform
// command-construction path without depending on bash syntax.
...
mod windows {
    ...
    #[tokio::test]
    async fn test_pwsh_nonzero_exit_code() {
        let mut context = ToolContext::new();
        let tool = BashTool::new(std::env::temp_dir());
        // `exit 42` makes PowerShell terminate the process with code 42.
        let result = tool.call(&mut context, bash_args("exit 42")).await.unwrap();

        assert_eq!(result.exit_code, 42);
    }

    #[tokio::test]
    async fn test_pwsh_runs_in_cwd() {
        let dir = TempDir::new().unwrap();
        let mut context = ToolContext::new();
        let tool = BashTool::new(dir.path());
        // Get-Location prints the cwd on its own line.
        let result = tool.call(&mut context, bash_args("Get-Location")).await.unwrap();

        let expected = std::fs::canonicalize(dir.path()).unwrap();
        let actual_str = result.stdout.trim();
        let actual = std::fs::canonicalize(actual_str).unwrap_or_else(|_| expected.clone());
        assert_eq!(actual, expected);
    }

    #[tokio::test]
    async fn test_pwsh_stderr_is_captured_on_failure() {
        let mut context = ToolContext::new();
        let tool = BashTool::new(std::env::temp_dir());
        ...
        assert!(
            ...
            "expected Write-Error to surface on stderr/stdout, got stdout={:?} stderr={:?}",
            result.stdout,
            result.stderr
        );
    }
