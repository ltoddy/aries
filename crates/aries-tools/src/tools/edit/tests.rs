...
async fn seed_file(ctx: &ToolContext, path: &std::path::Path, content: &str) {
    fs::write(path, content).await.expect("test operation should succeed");
...
async fn test_edit_simple_replacement() {
    ...
    assert_eq!(
        fs::read_to_string(&file_path).await.expect("test operation should succeed"),
        "hi world"
    );
}
...
async fn test_edit_rejects_unread_file() {
    ...
    fs::write(&file_path, "hello world").await.expect("test operation should succeed");
...
async fn test_edit_rejects_modified_since_read() {
    ...
    fs::write(&file_path, "hello brave world").await.expect("test operation should succeed");
