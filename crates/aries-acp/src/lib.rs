...
use agent_client_protocol::{Client, TextContent};
use anyhow::Result;
...
pub async fn execute() -> Result<()> {
    ...
    local_set
        .run_until(async move {
            ...
            let (conn, handle_io) =
                agent_client_protocol::AgentSideConnection::new(AriesAgent::new(tx), outgoing, incoming, |fut| {
...
struct AriesAgent {
    session_update_tx: mpsc::UnboundedSender<(agent_client_protocol::SessionNotification, oneshot::Sender<()>)>,
...
impl agent_client_protocol::Agent for AriesAgent {
    async fn initialize(
        ...
        Ok(agent_client_protocol::InitializeResponse::new(agent_client_protocol::ProtocolVersion::V1)
            .agent_info(agent_client_protocol::Implementation::new("aries", "0.1.0").title("Aries Agent")))
    }
    ...
    async fn new_session(
        ...
        Ok(agent_client_protocol::NewSessionResponse::new(session_id.to_string()))
    }
    ...
    async fn prompt(
        ...
        tracing::info!("Received prompt request {arguments:?}");

        let prompt_text: String = arguments
            .prompt
            .iter()
            .filter_map(|block| {
                if let agent_client_protocol::ContentBlock::Text(text) = block { Some(text.text.clone()) } else { None }
            })
            .collect::<Vec<_>>()
            .join("\n");
    ...
    async fn set_session_config_option(
    ...
    ) -> Result<agent_client_protocol::SetSessionConfigOptionResponse, agent_client_protocol::Error> {
        tracing::info!("Received set session config option request {args:?}");
        let value: agent_client_protocol::SessionConfigValueId = args.value;
        let option = agent_client_protocol::SessionConfigOption::select(
            args.config_id,
            "Example Option",
            value,
            vec![
        ...
        Ok(agent_client_protocol::SetSessionConfigOptionResponse::new(vec![option]))
    }
    ...
    async fn ext_method(
        ...
        tracing::info!("Received extension method call: method={}, params={:?}", args.method, args.params);
    ...
    async fn ext_notification(
        ...
        tracing::info!("Received extension notification: method={}, params={:?}", args.method, args.params);
        Ok(())
    }
}
