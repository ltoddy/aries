...
fn retriever_line_includes_type_and_description() {
    ...
    assert_eq!(mem.as_retriever_line(), "- [prefers_go.md] (user) — 偏好使用 Go");
}

#[test]
fn retriever_line_renders_each_memory_type() {
    assert_eq!(
        memory("a.md", "d", MemoryType::Feedback).as_retriever_line(),
        "- [a.md] (feedback) — d"
    );
    assert_eq!(
        memory("b.md", "d", MemoryType::Project).as_retriever_line(),
        "- [b.md] (project) — d"
    );
    assert_eq!(
        memory("c.md", "d", MemoryType::Reference).as_retriever_line(),
        "- [c.md] (reference) — d"
    );
}
